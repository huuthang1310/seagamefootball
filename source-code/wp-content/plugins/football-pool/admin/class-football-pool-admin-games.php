...
class Football_Pool_Admin_Games extends Football_Pool_Admin {
	...
	public static function admin() {
		...
		self::admin_header( __( 'Matches', 'football-pool' ), '', 'add new' );
			...
			if ( count( $header ) === count( $column_names ) ) {
				...
				if ( count( $err ) == 0 ) {
					...
					while ( ( $data = fgetcsv( $fp, 0, FOOTBALLPOOL_CSV_DELIMITER ) ) !== false ) {
						$row++; // start at 2, because first row contains header
	...
	private static function view() {
		...
		self::secondary_button( __( 'Bulk change game schedule', 'football-pool' ), 'schedule', false );
		self::secondary_button( 
			__( 'Download game schedule', 'football-pool' ), 
			$download_url, 
			false, 
			'link' 
		);
	...
	private static function edit( $item_id ) {
		...
		$cols = array(
					array( 'text', __( 'match date (UTC)', 'football-pool' ), 'match_date', $matchdate, sprintf( '<span title="%s">%s</span>', __( 'time of the match in local time (WordPress setting)', 'football-pool' ), sprintf( __( 'local time is %s', 'football-pool' ), $matchdate_local ) ) ),
		...
		if ( ! is_array( $rows ) || count( $rows ) == 0 ) {
			...
			foreach( $rows as $row ) {
				if ( $matchtype != $row['matchtype'] ) {
					...
					echo '<tr><td class="sidebar-name" colspan="8"><h3>', Football_Pool_Utils::xssafe( $matchtype ), '</h3></td></tr>';
				}
