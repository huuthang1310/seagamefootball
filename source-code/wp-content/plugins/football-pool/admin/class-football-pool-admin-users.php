...
class Football_Pool_Admin_Users extends Football_Pool_Admin {
	...
	public static function admin() {
		...
		if ( Football_Pool_Utils::get_fp_option( 'use_leagues', 0 ) == '1' ) {
			$league_id = Football_Pool_Utils::get_fp_option( 'default_league_new_user', 0 );
		...
		if ( $pool->has_leagues ) {
			...
			self::intro( __( 'To exclude users tick the appropiate column', 'football-pool' ) );
		}
	...
	public static function add_extra_profile_fields( $user ) {
		...
		if ( $pool->has_leagues ) {
			...
			if ( current_user_can( 'edit_users' ) ) {
				echo '<span class="description">', __( "<strong>Important:</strong> An administrator can change users in the plugin's admin page for", 'football-pool' ), ' <a href="admin.php?page=footballpool-users">', __( 'Users', 'football-pool' ), '</a>.</span>';
			}
			echo '</td></tr>';
