...
The Football Pool plugin is available in several languages. See https://translate.wordpress.org/projects/wp-plugins/football-pool for all available translations, the contributors or to help translating the plugin in your own language. I will keep including the original language files in the plugin until a language pack is available.
...
* [fp-ranking] shortcode: added value 'user' for league parameter to show the ranking for the league of the logged in user.
* [fp-user-ranking] shortcode: added value 'league_rank' to show the rank in the league of the selected user.
...
* Bug fix: Prediction form shortcode only checked for logged on users, not if they are a player in the pool (thanks ARTEFACTOCO for reporting this).
