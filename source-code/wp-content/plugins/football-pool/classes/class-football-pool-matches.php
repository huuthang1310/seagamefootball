...
class Football_Pool_Matches {
	...
	public function get_match_info_for_user( $user_id, $match_ids = array(), $all_matches = false ) {
		...
		if ( $all_matches === false ) $visible = 'WHERE t.visibility = 1';
	...
	public function print_matches( $matches, $page = '' ) {
		...
		// define templates
		$template_start = sprintf( '<table class="matchinfo %s">', $page );
		...
		foreach ( $matches as $row ) {
			...
			// translators: this is a date format string (see http://php.net/date)
			$match_day = date_i18n( _x( 'l', 'a date format string (see http://php.net/date)', 'football-pool' )
									, $localdate->format( 'U' ) );
	...
	public function print_matches_for_input( $matches, $form_id, $user_id, $is_user_page = false, $show_actual = false ) {
		...
		$match_type_template = apply_filters( 'footballpool_predictionform_match_type_template'
											, $match_type_template, $is_user_page );
		
		$date_row_template = '<tr><td class="matchdate" colspan="11">%match_datetime_formatted%</td></tr>';
		$date_row_template = apply_filters( 'footballpool_predictionform_date_row_template'
											, $date_row_template, $is_user_page );
		...
		$linked_question_template = apply_filters( 'footballpool_predictionform_linked_questions_template'
													, $linked_question_template, $is_user_page );
		...
		foreach ( $matches as $row ) {
			...
			$match_template_params = array(
				...
				'is_user_page' => $is_user_page,
				'home_actual_score' => ( $show_actual == true && is_numeric( $info['home_score'] ) ) 
											? sprintf( $actual_home_score, $info['home_score'] ) : '',
				'away_actual_score' => ( $show_actual == true && is_numeric( $info['away_score'] ) ) 
	...
	private function show_users_link( $match, $ts ) {
		...
		if ( $this->always_show_predictions || ! $this->match_is_editable( $ts ) ) {
			...
			$output .= sprintf( '<img src="%sassets/images/charts.png" alt="%s" title="%s" /></a>',
	...
	private function show_pool_joker( $joker, $match, $ts, $form_id = 1 ) {
		...
		*/
		...
		if ( $pool->has_jokers && ! $this->joker_blocked ) {
			if ( $this->match_is_editable( $ts ) ) {
				$add_joker = ' onclick="FootballPool.change_joker( this.id )" title="' . __( 'use your joker?', 'football-pool' ) . '"';
			}
		} else {
			$class .= ' readonly';
		}
