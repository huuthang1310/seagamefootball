...
class Football_Pool_Admin_Help extends Football_Pool_Admin {
	...
	public static function admin() {
		...
		<p>
		The Football Pool plugin installs a fantasy sports pool in your WordPress blog. In the default configuration this plugin enables you to define matches between (football) teams and lets your blog visitors predict the outcomes of the matches. Players earn points for correct predictions and the best player wins the pool.
		</p>
		<p>
		There are several ways you can customize the plugin: different scores for correct answers, add bonus questions, add your own rankings, etc. See the contents of this help file for details. If you have any questions, you can leave them at the <a target="_blank" href="http://wordpress.org/support/plugin/football-pool">WordPress forum</a>.
		</p>
		...
		<p>If you set the goal bonus to anything other than zero (default is zero), then this bonus is added to the scored points if the goals predicted are right; even if the match result was wrong (e.g. result is 2-1 and user predicted 1-1). Beware: these points are also rewarded double in case of a full score (win or draw), because the player has both amount of goals correct (see examples below).</p>
		...
		<p><em>* based on your current settings.</em></p>
		...
		<p>By default an admin will be automatically notified for a (re)calculation of the rankings when saving a match or question, or when changing your pool players. If you want to (temporarily) disable this automatic calculation, e.g. when you want to enter multiple matches at once or when you <a href="#ranking-calculation-wp-cli">use wp-cli</a> to do the calculation, you may disable this feature in the <a href="?page=footballpool-options">plugin options</a> and do a manual recalculation when you've finished editing.
		</p>
		<div class="help important">
...
define( \'WP_DEBUG_LOG\', true );');
		...
		<p>Plugin version 2.5.0 got a new calculation method. A calculation is being processed in a temporary table and once the calculation is finished, the temp table is activated. Only one calculation can run and if one is already running, other admins get a message that they will have to wait.
		If - for some reason - the plugin keeps telling you that a calculation is already running, but you are sure that this is wrong, you can override this message and still start a new one by adding the following constant in your wp-config.php file or by starting the calculation with the button below this paragraph (button is only active if calculation is stuck and AJAX requests are enabled).</p>
		<?php
		if ( Football_Pool_Utils::get_fp_option( 'calculation_in_progress' ) == 1 
			&& ! FOOTBALLPOOL_RANKING_CALCULATION_NOAJAX ) {
...
define( \'FOOTBALLPOOL_FORCE_CALCULATION\' , true );');
		?>
		<p>If the calculation stopped with an error, or if it was interrupted before it could finish, the above may also apply.
		</p>
		<h3 id="ranking-calculation-wp-cli">WP-CLI support</h3>
		<p>The ranking can also be calculated via the command-line if you have WP-CLI installed (see <a href="http://wp-cli.org" target="_blank">wp-cli.org</a> for details). This should also open the door for scheduling the calculation in a cron job. But as I have absolutely no experience with this :), I won't be able to help you with the configuration of crons.</p>
<pre class="code-block"><code class="bash">$ wp football-pool calc
Calculating scores  3  % [=>                                       ] 0:00 / 0:00
Calculating scores  7  % [===>                                     ] 0:08 / 1:47
Calculating scores  15 % [======>                                  ] 0:18 / 1:45
Calculating scores  23 % [=========>                               ] 0:23 / 0:35
Calculating scores  30 % [============>                            ] 0:27 / 0:38
Calculating scores  38 % [===============>                         ] 0:31 / 0:52
Calculating scores  46 % [==================>                      ] 0:38 / 1:08
Calculating scores  53 % [======================>                  ] 0:42 / 1:01
Calculating scores  61 % [=========================>               ] 0:45 / 0:24
Calculating scores  69 % [============================>            ] 0:49 / 0:47
Calculating scores  76 % [===============================>         ] 0:56 / 1:56
Calculating scores  84 % [==================================>      ] 1:00 / 0:50
Calculating scores  92 % [=====================================>   ] 1:04 / 0:58
...
<pre class="code-block"><code class="bash">$ wp football-pool calc --force-calculation</code></pre>
		...
		<p>In the user answer screen information about the question is shown as a reference (1). The question and default points are shown and - if an admin has filled in the answer - the answer is also shown.<br />
		For each player click the appropiate radiobutton for a right or wrong answer (2). If an answer is considered correct you have the option to give a different amount of points to that user (3). For example to give extra bonuspoints or to give half the points for an incomplete answer. Leave blank if you want to give the default points for the question.<br />
		...
		*/
		...
		or, when you want to give credits to the original author of the schedule when you only translated the team names, etc.:
		<pre class="code">
		/*
		 Contributor: Antoine Hurkmans
		 Translator: John Doe
		...
		*/
		...
		<p>Assets can be uploaded to the football-pool dir in the WP uploads dir <?php echo FOOTBALLPOOL_UPLOAD_DIR; ?>.</p>
		<h4>Minimal data</h4>
		<!--p>
		...
		<p>The matchtype can be entered in the following formats:
		<table>
		...
		<h3>[fp-countdown]</h3>
		<p>Counts down to a date and time. If no date is given, the time of the first match of the tournament is used. If a valid match number is given, it counts down to that match. A textual countdown is added to the post (or page) wich updates automatically. The shortcode uses a default format for the displayed text, but you can override it with the format_string parameter.</p>
		<p>
		<table class="widefat help">
			<tr><th>parameter</th><th>description</th><th>values</th><th>default</th></tr>
			<tr>
		...
		<p>Users, match and matchtype argument can be entered in the following formats (example for matches):
		<table>
			<tr><td>match 1</td><td>&rarr;</td><td>match="1"</td></tr>
			<tr><td>matches 1 to 5</td><td>&rarr;</td><td>match="1-5"</td></tr>
			<tr><td>matches 1, 3 and 6</td><td>&rarr;</td><td>match="1,3,6"</td></tr>
		...
		<p>example:<br />
		<span class="code">[fp-scores matchtype=1]</span><br />
		<span class="code">[fp-scores users="1-10" match="1,2"]</span><br />
		...
		<p>There is a <a target="_blank" href="https://wordpress.org/support/topic/extension-plugins-for-the-plugin-using-hooks">post on the support forum</a> where I placed some working examples of extension plugins that use some of the filters in the plugin.</p>
		<div class="help important">
...
?>' );
		...
		<p>Writing this plugin and maintaining it takes a lot of time. If you liked using this plugin please consider a <a href="https://paypal.me/wpfootballpool" target="_blank">small donation</a>.<br />
