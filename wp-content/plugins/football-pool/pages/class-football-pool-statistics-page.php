				...
				if ( $ranking_display == 1 && count( $user_defined_rankings ) > 0 ) {
					...
					if ( $ranking_display == 1 && count( $user_defined_rankings ) > 0 ) {
						...
						$ranking_selector .= sprintf( '<br />%s: %s'
											...
											, Football_Pool_Utils::select( 'ranking', 
																			$options, 
																			$ranking, 
																			'', 
				...
				case 'bonusquestion': 
					...
					if ( $stats->stats_visible ) {
						...
						if ( $stats->stats_enabled && $info['score_date'] != null ) {
							...
							$chart->title = __( 'what dit other users score?', 'football-pool' );
						...
						if ( count( $raw_data ) ) {
							...
							if ( $pool->has_jokers ) {
								$chart->options[] = sprintf( "subtitle: { text: '(%s)' }"
															, __( 'with the joker used', 'football-pool' ) );
							}
				...
				default:
					...
					if ( count( $users ) >= 1 ) {
						...
						if ( count( $raw_data ) > 0 ) {
							...
							$ordinal_suffixes = __( '["th", "st", "nd", "rd", "th"]', 'football-pool' );
